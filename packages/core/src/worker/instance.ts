...
export type ExtractWorkerConnParams<A extends AnyWorkerInstance> =
	A extends WorkerInstance<
		// biome-ignore lint/suspicious/noExplicitAny: Must be used for `extends`
		any,
		infer ConnParams,
...
export type ExtractWorkerConnState<A extends AnyWorkerInstance> =
	A extends WorkerInstance<
		// biome-ignore lint/suspicious/noExplicitAny: Must be used for `extends`
		any,
		// biome-ignore lint/suspicious/noExplicitAny: Must be used for `extends`
...
export class WorkerInstance<S, CP, CS, V, I, AD, DB> {
	 ...
	 */
	#persist!: PersistedWorker<S, CP, CS>;

	/** Raw state without the proxy wrapper */
	#persistRaw!: PersistedWorker<S, CP, CS>;
	 ...
	 *
	 * Constructed in {@link WorkerInstance.start}.
	 *
	 * @private
	 */
	...
	async start(
		...
		if (this.#config.onStart) {
			const result = this.#config.onStart(this.workerContext);
	...
	async scheduleEvent(
		...
		this.workerContext.log.info("scheduling event", {
			event: eventId,
			timestamp,
			action: fn,
		});
		...
		if (insertIndex === 0 || this.#persist.e.length === 1) {
			this.workerContext.log.info("setting alarm", { timestamp });
			await this.#workerDriver.setAlarm(this, newEvent.t);
		}
	}

	async onAlarm() {
		const now = Date.now();
		this.workerContext.log.debug("alarm triggered", {
			now,
		...
		if (runIndex === -1) {
			this.workerContext.log.debug("no events to run", { now });
			return;
		}
		...
		for (const event of scheduleEvents) {
			try {
				this.workerContext.log.info("running action for event", {
					event: event.e,
					timestamp: event.t,
					action: event.a,
					args: event.ar,
				...
				// Call function
				try {
					await fn.call(undefined, this.workerContext, ...event.ar);
				} catch (error) {
					this.workerContext.log.error("error while running event", {
				...
				this.workerContext.log.error("internal error while running event", {
					error: stringifyError(error),
					event: event.e,
					timestamp: event.t,
					action: event.a,
		...
		try {
			...
			if (this.#persistChanged) {
				...
				await this.#writePersistLock.lock(async () => {
					...
					// Write to KV
					await this.#workerDriver.writePersistedData(
						this.#workerId,
						this.#persistRaw,
					);
	 ...
	 */
	#setPersist(target: PersistedWorker<S, CP, CS>) {
		...
		this.#persist = onChange(
			...
			(path: string, value: any, _previousValue: any, _applyData: any) => {
				...
				if (this.#config.onStateChange && this.#ready) {
					try {
						this.#config.onStateChange(this.workerContext, this.#persistRaw.s);
					} catch (error) {
	...
	async #initialize() {
		// Read initial state
		const persistData = (await this.#workerDriver.readPersistedData(
			this.#workerId,
		)) as PersistedWorker<S, CP, CS>;

		if (persistData !== undefined) {
			logger().info("worker restoring", {
			...
			if (this.stateEnabled) {
				logger().info("worker state initializing");
			...
			const persist: PersistedWorker<S, CP, CS> = {
				s: stateData as S,
				c: [],
				e: [],
			};
			...
			await this.#workerDriver.writePersistedData(this.#workerId, persist);

			this.#setPersist(persist);

			// Notify creation
			if (this.#config.onCreate) {
				await this.#config.onCreate(this.workerContext, { input });
			}
		}
	}
	...
	async prepareConn(
		// biome-ignore lint/suspicious/noExplicitAny: TypeScript bug with ExtractWorkerConnParams<this>,
		params: any,
		request?: Request,
	): Promise<CS> {
	...
	async createConn(
		...
		if (this.#config.onConnect) {
			try {
				const result = this.#config.onConnect(this.workerContext, conn);
				if (result instanceof Promise) {
					deadline(
						result,
	...
	#assertReady() {
		if (!this.#ready) throw new errors.InternalError("Worker not ready");
	}
	...
	/**
	 * Returns a list of action methods available on this worker.
	 */
	get actions(): string[] {
	 ...
	 *
	 * This allows the worker runtime to ensure that a promise completes while
	...
	async stop() {
		if (this.isStopping) {
			logger().warn("already stopping worker");
			return;
		}
