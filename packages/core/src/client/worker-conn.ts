 ...
 *
 * @typedef {Function} WorkerErrorCallback
 */
export type WorkerErrorCallback = (error: errors.WorkerError) => void;
...
/**
 * Provides underlying functions for {@link WorkerConn}. See {@link WorkerConn} for using type-safe remote procedure calls.
 *
 * @see {@link WorkerConn}
 */
export class WorkerConnRaw {
	#disposed = false;
	...
	#errorHandlers = new Set<WorkerErrorCallback>();
	 ...
	 *
	 * Creates an instance of WorkerConnRaw.
	 *
	 * @protected
	 */
	public constructor(
	...
	/**
	 * Call a raw action connection. See {@link WorkerConn} for type-safe action calls.
	 *
	 * @see {@link WorkerConn}
...
enc
	...
	async #handleOnMessage(event: MessageEvent<any>) {
		...
		if ("i" in response.b) {
			...
			if (actionId) {
				...
				inFlight.reject(new errors.WorkerError(code, message, metadata));
			} else {
				...
				const workerError = new errors.WorkerError(code, message, metadata);

				// If we have an onOpenPromise, reject it with the error
				if (this.#onOpenPromise) {
					this.#onOpenPromise.reject(workerError);
				}

				// Reject any in-flight requests
				for (const [id, inFlight] of this.#actionsInFlight.entries()) {
					inFlight.reject(workerError);
					this.#actionsInFlight.delete(id);
				}

				// Dispatch to error handler if registered
				this.#dispatchWorkerError(workerError);
			}
	...
	#handleOnClose(event: Event | CloseEvent) {
		...
		if (!this.#disposed && !this.#connecting) {
			// TODO: Fetch worker to check if it's destroyed
	...
	#dispatchWorkerError(error: errors.WorkerError) {
	 ...
	 *
	 * @param {WorkerErrorCallback} callback - The callback function to execute when a connection error occurs.
	...
	#sendMessage(message: wsToServer.ToServer, opts?: SendHttpMessageOpts) {
		if (this.#disposed) {
			throw new errors.WorkerConnDisposed();
		}
	...
	async dispose(): Promise<void> {
		...
		this.#client[WORKER_CONNS_SYMBOL].delete(this);
...
/**
 * Connection to a worker. Allows calling worker's remote procedure calls with inferred types. See {@link WorkerConnRaw} for underlying methods.
 *
 * @example
 * ```
 * const room = client.connect<ChatRoom>(...etc...);
 * // This calls the action named `sendMessage` on the `ChatRoom` worker.
 ...
 * @see {@link WorkerConnRaw}
 */
export type WorkerConn<AD extends AnyWorkerDefinition> = WorkerConnRaw &
