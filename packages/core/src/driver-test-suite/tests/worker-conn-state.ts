...
import { CONN_STATE_APP_PATH, type ConnStateApp } from "../test-apps";

export function runWorkerConnStateTests(driverTestConfig: DriverTestConfig) {
	describe("Worker Connection State Tests", () => {
		...
		describe("Connection State Management", () => {
			test("should maintain unique state for each connection", async (c) => {
				...
				const conn2 = client.connStateWorker
					.getOrCreate([], {
			...
			test("should track connections in shared state", async (c) => {
				...
				// Get connection IDs tracked by the worker
