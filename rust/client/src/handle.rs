...
impl ActorHandleInner {
    pub(crate) fn new(endpoint: String) -> ActorHandle {
        Arc::new(Self {
            endpoint: endpoint.clone(),
            transport_driver: drivers::ws::WebSocketDriver::new(
                endpoint.clone()
            ),
    ...
    pub(crate) async fn connect(self: &Arc<Self>) -> Result<()> {
        ...
        tokio::spawn(async move {
            loop {
                let Ok(msg) = recver.recv_msg().await else {
                    eprintln!("Unexpected: driver.next_msg failed");
                    break;
                };
                ...
                let Ok(msg): Result<ToClient, _> = serde_json::from_slice(&msg) else {
