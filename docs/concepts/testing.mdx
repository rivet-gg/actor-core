...
To set up testing with RivetKit:
...
RivetKit's schedule functionality can be tested using Vitest's time manipulation utilities:
...
RivetKit's testing framework automatically handles server setup and teardown, so you can focus on writing effective tests for your business logic.
